...
export function formatConsoleArguments(type: Crdp.Runtime.ConsoleAPICalledEvent['type'], args: Crdp.Runtime.RemoteObject[], stackTrace?: StackTraceCodeFlow<IScript>): { args: Crdp.Runtime.RemoteObject[], isError: boolean } {
    switch (type) {
        case 'log':
        case 'debug':
        case 'info':
...
function stackTraceToString(stackTrace: StackTraceCodeFlow<IScript>): string {
    if (!stackTrace) {
        return '';
    }
