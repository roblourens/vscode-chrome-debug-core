...
export interface ITargetBreakpoints {
    ...
    removeBreakpoint(bpRecipie: BPRecipie<ScriptOrSourceOrURLOrURLRegexp>): Promise<void>;
}

interface BreakpointClass<TResource extends ScriptOrSourceOrURLOrURLRegexp> {
...
export class CDTPTargetBreakpoints extends CDTPEventsEmitterDiagnosticsModule<Crdp.DebuggerApi, void, Crdp.Debugger.EnableResponse> implements ITargetBreakpoints {
    ...
    public async setBreakpointByUrl(bpRecipie: BPRecipieInUrl<AlwaysBreak | ConditionalBreak>): Promise<BreakpointInUrl[]> {
        const condition = this.getBPRecipieCondition(bpRecipie);
        const url = bpRecipie.location.url.textRepresentation;
    ...
    public async setBreakpointByUrlRegexp(bpRecipie: BPRecipieInUrlRegexp<AlwaysBreak | ConditionalBreak>): Promise<BreakpointInUrlRegexp[]> {
        const condition = this.getBPRecipieCondition(bpRecipie);
        const urlRegex = bpRecipie.location.urlRegexp;
        const location = bpRecipie.location.coordinates;
    ...
    public async removeBreakpoint(bpRecipie: BPRecipie<ScriptOrSourceOrURLOrURLRegexp>): Promise<void> {
    ...
    private getBPRecipieCondition(bpRecipie: IBPRecipie<ScriptOrSourceOrURLOrURLRegexp, AlwaysBreak | ConditionalBreak>): string | undefined {
    ...
    private async toBreakpoinInResource<TResource extends ScriptOrSourceOrURLOrURLRegexp>(classToUse: BreakpointClass<TResource>,
    ...
    private async toBreakpointInUrl(bpRecipie: BPRecipieInUrl, actualLocation: Crdp.Debugger.Location): Promise<BreakpointInUrl> {
        return this.toBreakpoinInResource<URL<CDTPScriptUrl>>(BreakpointInUrl, bpRecipie, actualLocation);
    }
    ...
    public toLocationInScript(location: Crdp.Debugger.Location): Promise<LocationInScript> {
        return this._cdtpLocationParser.getPositionInScript(location);
    }
