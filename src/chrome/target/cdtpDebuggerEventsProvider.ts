...
export class CDTPDebuggerEventsProvider extends CDTPEventsEmitterDiagnosticsModule<Crdp.DebuggerApi, void, Crdp.Debugger.EnableResponse> implements ICDTPDebuggerEventsProvider {
    ...
    private getBPFromID(hitBreakpoint: Crdp.Debugger.BreakpointId): IBPRecipie<ScriptOrSourceOrURLOrURLRegexp> {
    ...
    private async toScope(scope: Crdp.Debugger.Scope): Promise<Scope> {
        return {
            ...
            startLocation: await asyncUndefinedOnFailure(async () => scope.startLocation && await this._cdtpLocationParser.getPositionInScript(scope.startLocation)),
            endLocation: await asyncUndefinedOnFailure(async () => scope.endLocation && await this._cdtpLocationParser.getPositionInScript(scope.endLocation))
        };
    }
