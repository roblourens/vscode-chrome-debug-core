...
import { IResourceIdentifier, newResourceIdentifierMap, parseResourceIdentifier } from './internal/resourceIdentifier';
...
export class BreakOnLoadHelper {
     ...
     */
    public async handleOnPaused(notification: PausedEvent): Promise<boolean> {
     ...
     */
    private async shouldContinueOnStopOnEntryBreakpoint(pausedLocation: LocationInScript): Promise<boolean> {
     ...
     * Only used when using regex approach for break on load
     */
    private async handleStopOnEntryBreakpointAndContinue(notification: MakePropertyRequired<PausedEvent, 'hitBreakpoints'>): Promise<boolean> {
