...
export interface IScript {
    runtimeSource: ILoadedSource; // Source in Webserver
...
export class Script implements IScript {
    private readonly _runtimeSource: ILoadedSource;
    ...
    public static create(locationInRuntimeEnvironment: IResourceLocation & CDTPScriptUrl, locationInDevelopmentEnvinronment: IResourceLocation,
        ...
        let runtimeSource: (script: IScript) => ILoadedSource;
        if (locationInRuntimeEnvironment.isEquivalent(locationInDevelopmentEnvinronment)) {
            if (fs.existsSync(locationInDevelopmentEnvinronment.textRepresentation)) {
                developmentSource = runtimeSource = script => new ScriptRunFromLocalStorage(script, locationInDevelopmentEnvinronment, 'TODO DIEGO');
            } else {
                developmentSource = runtimeSource = script => new DynamicScript(script, locationInDevelopmentEnvinronment, 'TODO DIEGO');
            }
        } else {
    ...
    public static createEval(name: ResourceName): Script {
    ...
    constructor(getRuntimeSource: (script: IScript) => ILoadedSource, getDevelopmentSource: (script: IScript) => ILoadedSource,
    ...
    public get runtimeSource(): ILoadedSource {
    ...
    public get allSources(): ILoadedSource[] {
        const unmappedSources = [this.runtimeSource];
