import { IResourceIdentifier, parseResourceIdentifier, parseResourceIdentifiers, newResourceIdentifierMap } from '../resourceIdentifier';
import { IToggleSkipFileStatusArgs, ICommonRequestArgs, utils, Crdp, BaseSourceMapTransformer } from '../../..';
import { logger } from 'vscode-debugadapter/lib/logger';
import { CallFramePresentation } from '../stackTraces';
...
import { StackTracesLogic } from '../stackTracesLogic';
...
export class SkipFilesLogic {
    ...
    */
    public async toggleSkipFileStatus(args: IToggleSkipFileStatusArgs): Promise<void> {
        ...
        const generatedPath = parseResourceIdentifier(await this._sourceMapTransformer.getGeneratedPathFromAuthoredPath(aPath.canonicalized));
        if (!generatedPath) {
            logger.log(`Can't toggle the skipFile status for: ${aPath} - haven't seen it yet.`);
            return;
        }

        const sources = parseResourceIdentifiers(await this._sourceMapTransformer.allSources(generatedPath.canonicalized));
    ...
    private async isInCurrentStack(args: IToggleSkipFileStatusArgs): Promise<boolean> {
        const currentStack = await this._stackTracesLogic.stackTrace({ threadId: undefined });

        return currentStack.stackFrames.some(frame => {
            return (frame instanceof CallFramePresentation)
            && frame.callFrame.location.source
             && frame.callFrame.location.source.identifier.isEquivalent(args.source);
        });
    }
        ...
        if (sources && sources.length) {
            ...
            for (let s of sources) {
                ...
                if ((isSkippedFile && !inLibRange) || (!isSkippedFile && inLibRange)) {
                    const details = await this._sourceMapTransformer.allSourcePathDetails(mappedUrl.canonicalized);
            ...
            if (libPositions.length || toggling) {
                ...
                await this._chrome.Debugger.setBlackboxedRanges(script, []).catch(() => this.warnNoSkipFiles());

                if (libPositions.length) {
                    this._chrome.Debugger.setBlackboxedRanges(script, libPositions).catch(() => this.warnNoSkipFiles());
                }
            }
        } else {
            ...
            if (typeof status === 'boolean' && status !== skippedByPattern) {
                ...
                this._chrome.Debugger.setBlackboxedRanges(script, positions).catch(() => this.warnNoSkipFiles());
            }
        }
    }
